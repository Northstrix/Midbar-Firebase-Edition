...
public class Editor {
    ...
    public static void gen_keys() {
	    ...
	    disp_rec("byte read_cards[16] = {\n");
	    for (int i = 0; i < 2; i++) {
	      SecureRandom crds_arr = new SecureRandom();
	      for (int j = 0; j < 8; j++) {
	        m = crds_arr.nextInt(2);
	        m1 = crds_arr.nextInt(2);
	        disp_rec("0x");
	        StringBuilder crds_arr_str = new StringBuilder();
	        if (m == 0)
	          crds_arr_str.append((char)(97 + (crds_arr.nextInt(6))));
	        if (m == 1)
	          crds_arr_str.append((char)(48 + (crds_arr.nextInt(10))));
	        if (m1 == 0)
	          crds_arr_str.append((char)(97 + (crds_arr.nextInt(6))));
	        if (m1 == 1)
	          crds_arr_str.append((char)(48 + (crds_arr.nextInt(10))));
	        crds_arr_str.append(",");
	        if (i == 1 && j == 7)
	          crds_arr_str.setLength(crds_arr_str.length() - 1);
	        disp_rec(crds_arr_str.toString());
	      }
	      disp_rec("\n");
	    }
	    disp_rec("};\n");
